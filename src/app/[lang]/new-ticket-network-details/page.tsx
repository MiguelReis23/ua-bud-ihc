...
export default function Home({
  ...
  const handleSubmit = async () => {
    const data = {
      subject: "Pedir Acesso Físico à Rede",
      details: `Unit (department): ${areaName}\nLocation: ${usage}\nSocket to Interviene: ${areaAdministrator}\nDescription: ${description}`,
    };
                ...
                <form className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <section className="flex flex-col col-span-2 md:col-span-1">
                    <div className="mb-4">
                      <Label
                        className="text-xl font-medium "
                        htmlFor="area-administrator"
                      >
                        Socket to interviene:*
                      </Label>
                      <Input
                        id="area-administrator"
                        placeholder="Enter area administrator"
                        onChange={(e) => setAreaAdministrator(e.target.value)}
                      />
                    </div>
